...
describe('Circular Reference Serialization', () => {
    ...
    it('should serialize circular references', () => {
        ...
        expect(serialCompany.employees[0].manager.manager[Cereal.idProperty]).to.equal(4);
        expect(serialCompany.employees[0].manager.manager.firstName).to.equal('C');
        expect(serialCompany.employees[0].manager.manager.manager).to.equal(2); // circular reference to A
