...
describe('Circular Reference Deserialization', () => {
    ...
    employees: [
      {
        ...
        firstName: 'A',
        lastName: 'Aa',
        manager: {'@id': 3, firstName: 'B', lastName: 'Bb', manager: 2}
      },
      3, {'@id': 4, firstName: 'C', lastName: 'Cc', manager: null},
      {'@id': 5, firstName: 'D', lastName: 'Dd', manager: 4}
    ]
  };
  ...
  it('should deserialize circular references', () => {
    ...
    expect(deserialCompany.employees[0].manager.manager)
        .to.equal(
            deserialCompany.employees[0]);  // Circular reference back to A

    expect(deserialCompany.employees[1].firstName).to.equal('B');
    expect(deserialCompany.employees[1].manager.firstName).to.equal('A');
    expect(deserialCompany.employees[1].manager)
        .to.equal(deserialCompany.employees[0]);
    expect(deserialCompany.employees[1].manager.manager)
        .to.equal(
            deserialCompany.employees[1]);  // Circular reference back to B

    expect(deserialCompany.employees[2].firstName).to.equal('C');

    expect(deserialCompany.employees[3].firstName).to.equal('D');
    expect(deserialCompany.employees[3].manager)
        .to.equal(deserialCompany.employees[2]);
  });
});