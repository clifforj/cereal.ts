 ...
 *
 * Exposed as 'Cereal'. Provides functions that serialize/deserialize objects
 * based on information from its meta store.
 */
export class SerializationService {
  ...
  /**
   * Serialize an object with the provided target. Valid targets are classes and
   * custom serializers. If no target is specified, this method will simply
  ...
  serialize(object: any, target?: Constructor<any>|ICustomSerializer) {
    ...
    if (target && !SerializationService.isCustomSerializer(target) && object) {
      if (Array.isArray(object)) {
        ...
        // Check if we've already serialized this object. If we have, then
        // output the objects ID instead
        if (this.serializationIdMap.has(object) &&
            this.supportCircularReferences) {
          serializedObject = this.serializationIdMap.get(object);

          // This is the first time we've encountered this object, serialise it
          // and add it to the map with an ID
        } else {
          serializedObject = {};
          ...
          SerializationService.callLifeCycleHook(
              serializedObject, object, target, true, false);
          ...
          classMeta.properties.forEach((propMeta) => {
            ...
            // If an override property name is specified, use it. Otherwise use
            // the default.
          ...
          SerializationService.callLifeCycleHook(
              serializedObject, object, target, true, true);
        }
      }
      // Targeting a ICustomSerializer
    } else if (
        target && SerializationService.isCustomSerializer(target) &&
        target.serialize) {
  ...
  /**
   * Deserialize an object with the provided target. Valid targets are classes
   * and custom serializers. If no target is specified, this method will simply
   * return what it was given.
   *
  ...
  deserialize(object: any, target?: Constructor<any>|ICustomSerializer) {
    ...
    if (target && !SerializationService.isCustomSerializer(target) && object) {
      if (Array.isArray(object)) {
        ...
        // If the passed value is a number when we're expecting to deserialise
        // an object, it's likely an ID
        if (!isNaN(object) && this.deserializationIdMap.has(object) &&
            this.supportCircularReferences) {
          ...
          if (this.supportCircularReferences) {
            this.deserializationIdMap.set(
                object[this.idProperty], deserializedObject);
          }

          SerializationService.callLifeCycleHook(
              deserializedObject, object, target, false, false);
          classMeta.properties.forEach((propMeta) => {
            ...
            const deserializedPropertyName =
                deserializationMeta.propertyNameOverride ||
          ...
          SerializationService.callLifeCycleHook(
              deserializedObject, object, target, false, true);
        }
      }
      // Targeting a ICustomSerializer
    } else if (
